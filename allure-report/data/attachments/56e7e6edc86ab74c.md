...
- Location: tests/mobile/mobileApp.spec.js:3:1
...
```ts
  ...
  2  | 
  3  | test('Mobile app basic test', async ({ page }) => {
  4  |   console.log('🚀 Launching mobile test...');
  5  | 
  ...
  13 | 
  14 |   // Basic element check (Google logo)
  15 |   const logo = page.locator('img[alt="Google"]');
> 16 |   await expect(logo).toBeVisible();
     |                      ^ Error: expect(locator).toBeVisible() failed
  17 | 
  18 |   console.log('✅ Mobile test passed');
  19 | });
```